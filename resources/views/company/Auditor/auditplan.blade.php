												  ...
												  @if($companyplan->circulate==='0')
												   ...
												   @endif
												    @if($companyplan->circulate==='1')
													 Audit Plan Circulated
												   @endif
											
											   </td>
										<td><a href="./auditor_planview/{{$companyplan->company}}/{{$companyplan->audit_number}}" class="btn btn-sm btn-icon btn-pure btn-outline delete-row-btn" data-toggle="tooltip" data-original-title="View Audit Plan"><i class="ti-eye" aria-hidden="true"></i></a>
										</td>
										</td>
									</tr>
									@endforeach
