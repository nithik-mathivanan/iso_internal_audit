										...
										<td  class="border1"> 
											<select class="required dep{{$sno}}" name="auditor[]"  placeholder="Select Auditor" id="aud{{$sno}}" style=" width: auto;">
												...
												@php } 
													...
													<option value="{{$companyauditor->id}}">{{$companyauditor->name}}	</option>
													@endforeach
...
<form action="{{ route('AuditPlanPdf') }}" style="display: inline-block;">
	<input value="{{request()->route('companyid')}}" name="companyid" style="display: none;">
	<input value="{{request()->route('audit_number')}}" name="audit_number" style="display:none ;">
...
var current_id = $(this).attr('id').split('p')[1];
var id= current_id-1;
...
for(var i=0;i<data1.length;i++){
 op2+='	<label class="custom-control custom-checkbox "><input type="checkbox" name="auditee['+id+']['+i+']"  class="custom-control-input" aria-invalid="false" value="'+data1[i].id+'" ><span class="custom-control-label">'+data1[i].name+'</span></label>';
