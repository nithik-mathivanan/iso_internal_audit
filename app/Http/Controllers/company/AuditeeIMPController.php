...
class AuditeeIMPController extends Controller
{
	public function view($AuditPlanNo)
	{
		...
		$program = CImprovement::where('status',1)->where('company_id',$company_id)->where('AuditPlanNo',$AuditPlanNo)->get();
      
		
		return view('company.Auditee.IMP.view',compact('program'))->with('sno',$sno=1);
    }
     public function impview($AuditPlanNo){ 
                ...
                return view('company.Auditee.IMP.view',compact('report'))->with('sno',$sno=1);
     }
    public function IMPviewdetail($AuditPlanNo,Request $request){ 
       ...
       //  $report = Cauditreport::select('AuditPlanNo','company_id','plan_date')->distinct()->where('company_id',$company_id)->where('auditee',$id)->offset($start)->limit($limit);
        ...
        ->addColumn('impreport', function ($user){       
                //$program = CReport::select('IMPNO')->where('IMPNO',$user->id)->get();
        ...
        ->editColumn('status',
         ...
         ->rawColumns(['action','status','impreport'])
         ->setTotalRecords($totalrecords)
         ->make(true);
    }
    public function impeffective($id,$AuditPlanNo){ 
        $user = User::get();
     ...
     return view('company.Auditee.IMP.report',compact('ncr','auditplan','report','user'))->with('sno',$sno=1);
